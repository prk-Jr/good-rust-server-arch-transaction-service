...
# Create account
ALICE_RESP=$(curl -s -X POST "$BASE_URL/api/accounts" -H "Content-Type: application/json" -d '{"name":"Alice","currency":"USD"}')
...
# Get account
GET_RESP=$(curl -s "$BASE_URL/api/accounts/$ALICE_ID")
assert_contains "Get account by ID" "Alice" "$GET_RESP" && TESTS_PASSED=$((TESTS_PASSED + 1)) || TESTS_FAILED=$((TESTS_FAILED + 1))

# List accounts
LIST_RESP=$(curl -s "$BASE_URL/api/accounts")
assert_contains "List accounts" "Alice" "$LIST_RESP" && TESTS_PASSED=$((TESTS_PASSED + 1)) || TESTS_FAILED=$((TESTS_FAILED + 1))
...
DEP_RESP=$(curl -s -X POST "$BASE_URL/api/transactions/deposit" -H "Content-Type: application/json" \
...
BALANCE=$(curl -s "$BASE_URL/api/accounts/$ALICE_ID" | grep -o '"amount":[0-9]*' | cut -d':' -f2)
...
WITH_RESP=$(curl -s -X POST "$BASE_URL/api/transactions/withdraw" -H "Content-Type: application/json" \
...
BOB_RESP=$(curl -s -X POST "$BASE_URL/api/accounts" -H "Content-Type: application/json" -d '{"name":"Bob","currency":"USD"}')
BOB_ID=$(echo "$BOB_RESP" | grep -o '"id":"[^"]*"' | cut -d'"' -f4)

XFER_RESP=$(curl -s -X POST "$BASE_URL/api/transactions/transfer" -H "Content-Type: application/json" \
...
ALICE_BAL=$(curl -s "$BASE_URL/api/accounts/$ALICE_ID" | grep -o '"amount":[0-9]*' | cut -d':' -f2)
BOB_BAL=$(curl -s "$BASE_URL/api/accounts/$BOB_ID" | grep -o '"amount":[0-9]*' | cut -d':' -f2)
...
EUR_RESP=$(curl -s -X POST "$BASE_URL/api/accounts" -H "Content-Type: application/json" -d '{"name":"Euro Account","currency":"EUR"}')
EUR_ID=$(echo "$EUR_RESP" | grep -o '"id":"[^"]*"' | cut -d'"' -f4)
curl -s -X POST "$BASE_URL/api/transactions/deposit" -H "Content-Type: application/json" \
    -d "{\"account_id\":\"$EUR_ID\",\"amount\":5000,\"currency\":\"EUR\"}" >/dev/null

CROSS_RESP=$(curl -s -X POST "$BASE_URL/api/transactions/transfer" -H "Content-Type: application/json" \
